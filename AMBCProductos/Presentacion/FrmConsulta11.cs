    ...
    public partial class FrmConsulta11 : Form
    {
        ...
        private void btnConsultar2_Click(object sender, EventArgs e)
        {
            if (Validar())
            {
                ...
                string consultasql = $"SELECT pr.nombre AS Producto,i.cantidad AS Stock, cat.nombre AS Categoria, pr.peso_kg AS Peso, s.nombre AS Sucursal, tp.nombre AS Tipo_Producto FROM inventarios i JOIN productos pr ON i.producto_id = pr.id_producto JOIN categorias cat ON pr.categoria_id = cat.id_categoria JOIN tipos_productos tp ON pr.tipo_producto_id = tp.id_tipo_producto JOIN sucursales s ON i.sucursal_id = s.id_sucursal WHERE i.cantidad = {valorSeleccionado} AND cat.nombre IN ('Gourmet', 'Fiambres', 'Lacteos') -- Conjunto AND pr.peso_kg IS NOT NULL -- Valor no nulo ORDER BY i.cantidad DESC;";
        ...
        private bool Validar()
        {
            ...
            if (valorSeleccionado < 100 || valorSeleccionado > 300)
            {
        ...
        private void nmPeso_ValueChanged(object sender, EventArgs e)
        {
